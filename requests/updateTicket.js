...
exports.handler = async (event, context, callback) => {
    ...
    //check if the a ticket with the passed id exists
    const searchParams = {
        TableName: tableName
    };
    const data = await documentClient.scan(searchParams).promise()
    const dataFiltered = data.Items.filter(ticket => ticket.id === event.ticket.id)
